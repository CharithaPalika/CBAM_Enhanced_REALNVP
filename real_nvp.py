...
class WeightNormConv2d(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride=1, padding=0, 
        bias=True, weight_norm=True, scale=False, use_cbam = False):
...
class ResidualBlock(nn.Module):
    def __init__(self, dim, bottleneck, weight_norm, use_cbam = False):
        ...
        if bottleneck:
            self.res_block = nn.Sequential(
                WeightNormConv2d(dim, dim, (1, 1), stride=1, padding=0, 
                    bias=False, weight_norm=weight_norm, scale=False, use_cbam=use_cbam),
                nn.BatchNorm2d(dim),
                nn.ReLU(),
                ...
                nn.ReLU(),
                WeightNormConv2d(dim, dim, (1, 1), stride=1, padding=0, 
                    bias=True, weight_norm=weight_norm, scale=True, use_cbam=use_cbam))
        else:
            self.res_block = nn.Sequential(
                WeightNormConv2d(dim, dim, (3, 3), stride=1, padding=1, 
                    bias=False, weight_norm=weight_norm, scale=False,use_cbam=use_cbam),
...
class ResidualModule(nn.Module):
    def __init__(self, in_dim, dim, out_dim, 
        res_blocks, bottleneck, skip, weight_norm, use_cbam = False):
        ...
        if res_blocks > 0:
            self.in_block = WeightNormConv2d(in_dim, dim, (3, 3), stride=1, 
                padding=1, bias=True, weight_norm=weight_norm, scale=False, use_cbam=use_cbam)
            self.core_block = nn.ModuleList(
                [ResidualBlock(dim, bottleneck, weight_norm, use_cbam=use_cbam) 
            ...
            if skip:
                self.in_skip = WeightNormConv2d(dim, dim, (1, 1), stride=1, 
                    padding=0, bias=True, weight_norm=weight_norm, scale=True, use_cbam=use_cbam)
                self.core_skips = nn.ModuleList(
                    [WeightNormConv2d(
                        dim, dim, (1, 1), stride=1, padding=0, bias=True, 
                        weight_norm=weight_norm, scale=True, use_cbam=use_cbam) 
                    for _ in range(res_blocks)])
        else:
            if bottleneck:
    ...
    def __init__(self, in_out_dim, mid_dim, size, mask_config, hps):
        ...
        self.block = nn.Sequential(
            ...
            ResidualModule(2*in_out_dim+1, mid_dim, in_out_dim, 
                self.res_blocks, self.bottleneck, self.skip, self.weight_norm, use_cbam=hps.use_cbam))
...
class ChannelwiseAdditiveCoupling(AbstractCoupling):
    ...
    def forward(self, x, reverse=False):
        ...
        log_diag_J = torch.zeros_like(shift)    # unit Jacobian determinant
        
        # See Eq(3) and Eq(4) in NICE and Section 3.7 in real NVP
        if reverse:
            if self.coupling_bn:
                mean, var = self.out_bn.running_mean, self.out_bn.running_var # C//2
                mean = mean.reshape(-1, 1, 1, 1).transpose(0, 1)
                var = var.reshape(-1, 1, 1, 1).transpose(0, 1)
